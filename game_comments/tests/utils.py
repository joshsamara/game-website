...
class BaseViewTestCase(BaseTestCase):
    ...
    def _assertLogin(self, required, verb, *args, **kwargs):
        """Check whether a login is required for the view."""
        self.client.logout()
        if verb == 'get':
            response = self.client.get(self.url(*args, **kwargs))
        else:
            response = self.client.post(self.url(*args, **kwargs))
    ...
    def assertLoginRequired(self, *args, **kwargs):
        ...
        self._assertLogin(True, 'get', *args, **kwargs)

    def assertNoLoginRequired(self, *args, **kwargs):
        """Assert login isn't required."""
        self._assertLogin(False, 'get', *args, **kwargs)

    def assertLoginRequiredPost(self, *args, **kwargs):
        """Assert login is required for a post"""
        self._assertLogin(True, 'post', *args, **kwargs)
